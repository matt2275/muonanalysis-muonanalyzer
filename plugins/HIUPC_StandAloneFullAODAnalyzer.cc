...
class HIUPC_StandAloneMuonFullAODAnalyzer : public edm::one::EDAnalyzer<> {
public:
  typedef std::vector<std::pair<reco::Muon, reco::TransientTrack>> RecoTrkAndTransientTrkCollection;
  explicit HIUPC_StandAloneMuonFullAODAnalyzer(const edm::ParameterSet&);
  ~HIUPC_StandAloneMuonFullAODAnalyzer() override;
...
private:
  ...
  edm::EDGetToken calotowerToken_;  
...
//
HIUPC_StandAloneMuonFullAODAnalyzer::HIUPC_StandAloneMuonFullAODAnalyzer(const edm::ParameterSet& iConfig)
    :  // inputs
...
HIUPC_StandAloneMuonFullAODAnalyzer::~HIUPC_StandAloneMuonFullAODAnalyzer() {
  // cout << "total " << trg_counter << " fires " << fire_counter << " l3"
  // << l3_counter << endl; do anything here that needs to be done at
  // desctruction time
}

bool HIUPC_StandAloneMuonFullAODAnalyzer::HLTaccept(const edm::Event& iEvent,
                                              NtupleContent& nt,
...
void HIUPC_StandAloneMuonFullAODAnalyzer::fillHLTmuon(const edm::Event& iEvent,
  ...
  for (auto ifilter : HLTFilters) {
    ...
    if (filterIndex < (*triggerObjects).sizeFilters()) {
      ...
      for (size_t j = 0; j < keys.size(); j++) {
        ...
        if (!(fabs(foundObject.id()) == 13 || ((fabs(foundObject.id()) == 0) && (filter_name.Contains("hltL1sSingleMu") || filter_name.Contains("hltL1MuOpen")))))
          continue;
...
void HIUPC_StandAloneMuonFullAODAnalyzer::embedTriggerMatching(const reco::Muon& mu,
                                                         std::vector<TString>& trg_filter,
                                                         std::vector<float>& trg_pt,
                                                         std::vector<float>& trg_eta,
                                                         std::vector<float>& trg_phi,
...
void HIUPC_StandAloneMuonFullAODAnalyzer::StandAlone_embedTriggerMatching(const reco::Muon& mu,
                                                                    std::vector<TString>& trg_filter,
                                                                    std::vector<float>& trg_pt,
                                                                    std::vector<float>& trg_eta,
                                                                    std::vector<float>& trg_phi,
...
//
void HIUPC_StandAloneMuonFullAODAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace std;
  using namespace edm;
  ...
  // iEvent.getByToken(centToken_, centrality);
  ...
  //commented out to save space, added useful details for HIUPC collisions
    ...
    // StandAlone_nt.nPFCands = pfcands->size();
    // for (const auto &pfc : *pfcands) {
    // //std::cout<< "pfc id " << pfc.pdgId()<< " from PV "<< pfc.fromPV() <<std::endl;
      // StandAlone_nt.pfcand_pdgId.push_back(pfc.pdgId());
      // StandAlone_nt.pfcand_charge.push_back(pfc.charge()); 
      // StandAlone_nt.pfcand_pt.push_back(pfc.pt());
      // StandAlone_nt.pfcand_eta.push_back(pfc.eta());
      // StandAlone_nt.pfcand_phi.push_back(pfc.phi());  
   
    // }
  
  
    // StandAlone_nt.nTower = calotowercollection->size();
  // for (edm::SortedCollection<CaloTower>::const_iterator calo = calotowercollection->begin(); calo != calotowercollection->end(); ++calo) {

       // StandAlone_nt.CaloTower_emE.push_back(calo->emEnergy());
       // StandAlone_nt.CaloTower_hadE.push_back(calo->hadEnergy());
       // StandAlone_nt.CaloTower_e.push_back(calo->energy());
       // StandAlone_nt.CaloTower_et.push_back(calo->et());
       // StandAlone_nt.CaloTower_phi.push_back(calo->phi());
       // StandAlone_nt.CaloTower_eta.push_back(calo->eta());
  ...
  // }
    ...
    // for (auto const& rh : *zdcrechits) {
      ...
      // if (nhits  < 18) {
       // StandAlone_nt.ZDC_e[nhits] = rh.energy();
       // StandAlone_nt.ZDC_zside[nhits] = zdcid.zside();
       // StandAlone_nt.ZDC_section[nhits] = zdcid.section();
       // StandAlone_nt.ZDC_channel[nhits] = zdcid.channel();
       // StandAlone_nt.ZDC_PM_Total_Energy+= rh.energy()  ;
       // if(zdcid.zside() >0 ){
       // StandAlone_nt.ZDC_P_Total_Energy += rh.energy();
       // if(zdcid.section() == 1) StandAlone_nt.ZDC_P_ECal_Energy += rh.energy();
       // if(zdcid.section() == 2) StandAlone_nt.ZDC_P_HCal_Energy += rh.energy()  ;        
       // }
       // if(zdcid.zside() <0 ){
       // StandAlone_nt.ZDC_M_Total_Energy += rh.energy();
       // if(zdcid.section() == 1) StandAlone_nt.ZDC_M_ECal_Energy += rh.energy();
       // if(zdcid.section() == 2) StandAlone_nt.ZDC_M_HCal_Energy += rh.energy() ;         
       // }
...
// ------------
void HIUPC_StandAloneMuonFullAODAnalyzer::beginJob() {
...
// ------------
void HIUPC_StandAloneMuonFullAODAnalyzer::endJob() {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void HIUPC_StandAloneMuonFullAODAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(HIUPC_StandAloneMuonFullAODAnalyzer);
