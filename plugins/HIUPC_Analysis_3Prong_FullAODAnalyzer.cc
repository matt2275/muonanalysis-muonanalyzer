...
class HIUPC_Analysis_3Prong_FullAODAnalyzer : public edm::one::EDAnalyzer<> {
public:
...
static bool compare_Tau_3prong_data_pt(tau_cand_3prong i1, tau_cand_3prong i2)
{
  ...
  explicit HIUPC_Analysis_3Prong_FullAODAnalyzer(const edm::ParameterSet&);
  ~HIUPC_Analysis_3Prong_FullAODAnalyzer() override;
...
//
HIUPC_Analysis_3Prong_FullAODAnalyzer::HIUPC_Analysis_3Prong_FullAODAnalyzer(const edm::ParameterSet& iConfig)
    :  // inputs
...
HIUPC_Analysis_3Prong_FullAODAnalyzer::~HIUPC_Analysis_3Prong_FullAODAnalyzer() {
  ...
  float HIUPC_Analysis_3Prong_FullAODAnalyzer::Find3ProngArea( float prong1_eta, float prong1_phi,
...
bool HIUPC_Analysis_3Prong_FullAODAnalyzer::HLTaccept(const edm::Event& iEvent,
                                              Analysis_NtupleContent& nt,
...
void HIUPC_Analysis_3Prong_FullAODAnalyzer::fillHLTmuon(const edm::Event& iEvent,
...
void HIUPC_Analysis_3Prong_FullAODAnalyzer::embedTriggerMatching(const reco::Track& mu,
                                                         std::vector<TString>& trg_filter,
                                                         std::vector<float>& trg_pt,
                                                         std::vector<float>& trg_eta,
                                                         std::vector<float>& trg_phi,
...
void HIUPC_Analysis_3Prong_FullAODAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace std;
  using namespace edm;
  ...
  while (!pair_Mass_Mmumu.empty()) {
    ...
    for (unsigned int tau_probe_idx = 0; tau_probe_idx <  Tau_Cands.size(); tau_probe_idx++) {
     ...
     trk_pair.insert(trk_pair.begin(),tag.second);
      ...
      // // save quantities to ordered heap
      auto pair_idx = std::make_pair(tag_idx, tau_probe_idx);
...
nt.npairs_3prong = TnPSize_3prong;    
  ...
  // assign sorted vtx indices to ranking
  map<std::pair<int, int>, int> pair_rank_vtx_prob_3prong ={};
  ...
  map<std::pair<int, int>, int> pair_rank_dPhi_muons_3prong ={};
  ...
  map<std::pair<int, int>, int> pair_rank_Mass_Mmumu_3prong ={};
    ...
    for (unsigned int tau_probe_idx = 0; tau_probe_idx <  Tau_Cands.size(); tau_probe_idx++) {
     ...
     trk_pair.insert(trk_pair.begin(),tag.second);
      ...
      Analysis_KlFitter vtx(trk_pair);
        ...
        nt.iprobe_3prong++;
        nt.pair_rank_vtx_prob_3prong = pair_rank_vtx_prob_3prong [{tag_idx, tau_probe_idx}];
        nt.pair_rank_dPhi_muons_3prong = pair_rank_dPhi_muons_3prong [{tag_idx, tau_probe_idx}];
        nt.pair_rank_Mass_Mmumu_3prong = pair_rank_Mass_Mmumu_3prong [{tag_idx, tau_probe_idx}];
        t4->Fill();
        if(saveCutTree_) t2->Fill();
   }
}
...
// ------------
void HIUPC_Analysis_3Prong_FullAODAnalyzer::beginJob() {
...
// ------------
void HIUPC_Analysis_3Prong_FullAODAnalyzer::endJob() {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void HIUPC_Analysis_3Prong_FullAODAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(HIUPC_Analysis_3Prong_FullAODAnalyzer);
