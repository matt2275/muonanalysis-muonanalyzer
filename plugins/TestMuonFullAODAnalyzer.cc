...
class TestMuonFullAODAnalyzer : public edm::one::EDAnalyzer<> {
public:
  typedef std::vector<std::pair<reco::Muon, reco::TransientTrack>> RecoTrkAndTransientTrkCollection;
  explicit TestMuonFullAODAnalyzer(const edm::ParameterSet&);
  ~TestMuonFullAODAnalyzer() override;
...
private:
  ...
  const double SA_maxdr_trk_mu_;
  const double SA_minpt_trk_;
...
//
TestMuonFullAODAnalyzer::TestMuonFullAODAnalyzer(const edm::ParameterSet& iConfig)
    :  // inputs
      ...
      SA_maxdr_trk_mu_(iConfig.getParameter<double>("maxDRProbeTrkSA")),
      SA_minpt_trk_(iConfig.getParameter<double>("minPtTrkSA")), 
...
TestMuonFullAODAnalyzer::~TestMuonFullAODAnalyzer() {
  // cout << "total " << trg_counter << " fires " << fire_counter << " l3"
  // << l3_counter << endl; do anything here that needs to be done at
  // desctruction time
}

bool TestMuonFullAODAnalyzer::HLTaccept(const edm::Event& iEvent, NtupleContent& nt, std::vector<std::string>& HLTPaths) {
...
void TestMuonFullAODAnalyzer::fillHLTmuon(const edm::Event& iEvent,
...
void TestMuonFullAODAnalyzer::embedTriggerMatching(const reco::Muon& mu,
                                               std::vector<TString>& trg_filter,
                                               std::vector<float>& trg_pt,
                                               std::vector<float>& trg_eta,
                                               std::vector<float>& trg_phi,
                                               std::vector<std::string>& HLTFilters,
                                               bool isTag, bool isStandAlone = false,
                                               const int& debug_ = 0) {
...
//
void TestMuonFullAODAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  using namespace std;
  using namespace edm;
...
//SA
  ...
  for (const auto& mu : *muons) {
    ...
    for (const reco::Track& trk : *tracks) {
      if (mu.charge() != trk.charge() and false ) // SA not active
        continue;
  ...
  //SA probe global muons mapping with StandAlone muon object
  std::pair<std::vector<unsigned>, std::vector<unsigned>> GlobeMuon_SAmuon_map;
  for (const auto& mu : *SAmuons) {
    float minDR = 1000;
    unsigned int idx_trk;
    if (debug_ > 0)
      std::cout << "Standalone New globeMu-SAmuon map entry pt " << mu.pt() << " eta " << mu.eta() << " phi " << mu.phi() << " vz " << mu.vz() << std::endl;
    for (const auto& tmp_mu : *muons) {
      if (!tmp_mu.isStandAloneMuon())
        continue;
       const reco::Track Globe_mu = *tmp_mu.standAloneMuon();
      if (debug_ > 1)
         std::cout << "Global Muon New globeMu-SAmuon map entry pt " << Globe_mu.pt() << " eta " << Globe_mu.eta() << " phi " << Globe_mu.phi()<< " vz " << Globe_mu.vz() << std::endl;
      if (mu.charge() != Globe_mu.charge() and false ) // SA not active
        continue;
//      if (!Globe_mu.isGlobalMuon())
//        continue;
      float DR = deltaR(mu.eta(), mu.phi(), Globe_mu.eta(), Globe_mu.phi());
      if (debug_ > 1)
        std::cout << "   DR " << DR << "  " << mu.eta() << "  " << mu.phi() << "  " << Globe_mu.eta() << "  " << Globe_mu.phi()
                  << std::endl;
            
      if (minDR < DR)
        continue;
      minDR = DR;
      idx_trk = &tmp_mu - &muons->at(0);
    }

//    if (minDR > SA_maxdr_trk_mu_)
    if (minDR > 100)
      continue;
   
    GlobeMuon_SAmuon_map.first.push_back(idx_trk);
    GlobeMuon_SAmuon_map.second.push_back(&mu - &SAmuons->at(0));
  }
  if (debug_ > 0)
    std::cout << "Matched Standalone globeMu-SAmuon " << GlobeMuon_SAmuon_map.first.size() << std::endl;
  ...
  for (const auto& tmp_mu : *muons) {
    ...
    unsigned int idx_trk;
    if (debug_ > 0)
      std::cout << "Standalone New trk-muon map entry pt " << mu.pt() << " eta " << mu.eta() << " phi " << mu.phi() << " vz " << mu.vz() << std::endl;
    for (const reco::Track& trk : *tracks) {
      if (debug_ > 1)
         std::cout << "Trk New trk-muon map entry pt " << trk.pt() << " eta " << trk.eta() << " phi " << trk.phi()<< " vz " << trk.vz() << std::endl;
      // if (mu.charge() != trk.charge())
        // continue;
      if (mu.pt()/trk.pt() > 5 or trk.pt()/mu.pt() > 5)
        continue;    
      if (trk.pt() < SA_minpt_trk_)
        continue;
      // if (fabs(mu.vz() - trk.vz()) > maxdz_trk_mu_ && maxdz_trk_mu_ > 0)
        // continue;
      // if (fabs(mu.pt() - trk.pt()) / mu.pt() > maxpt_relative_dif_trk_mu_ && maxpt_relative_dif_trk_mu_ > 0)
...
//    if (minDR > SA_maxdr_trk_mu_)
  ...
  while (!SA_pair_vtx_probs.empty()) {
    SA_pair_rank_vtx_prob[SA_pair_vtx_probs.top().second] = SA_pair_rank_vtx_prob.size();  // careful: RHS evaluated first
  ...
  while (!SA_pair_dPhi_muons.empty()) {
    SA_pair_rank_dPhi_muons[SA_pair_dPhi_muons.top().second] = SA_pair_rank_dPhi_muons.size();  // careful: RHS evaluated first
    SA_pair_dPhi_muons.pop();
  }
  map<std::pair<int, int>, int> SA_pair_rank_dM_Z_Mmumu;
  while (!SA_pair_dM_Z_Mmumu.empty()) {
    SA_pair_rank_dM_Z_Mmumu[SA_pair_dM_Z_Mmumu.top().second] = SA_pair_rank_dM_Z_Mmumu.size();  // careful: RHS evaluated first
  ...
  for (auto& tag : tag_trkttrk) {
    ...
    for (const reco::Muon &tmp_probe : *muons) {
      ...
      embedTriggerMatching(tag.first, nt.trg_filter, nt.trg_pt, nt.trg_eta, nt.trg_phi, tagFilters_, true, true, debug_);
...
//      auto Globe_it = std::find(GlobeMuon_SAmuon_map.second.begin(), GlobeMuon_SAmuon_map.second.end(), &probe - &SAmuons->at(0));
      reco::Muon fakeMuon;
      fakeMuon.setP4(mu2);
...
//         match_muon_idx  = SA_trk_muon_map.second[idx];
      }
      // if(Globe_it != GlobeMuon_SAmuon_map.second.end()){
         // unsigned global_idx = std::distance(GlobeMuon_SAmuon_map.second.begin(), Globe_it);
         // match_muon_idx = GlobeMuon_SAmuon_map.first[global_idx];
      // }
      StandAloneFillProbeBranches<reco::Muon, reco::Muon, reco::Track>(fakeMuon, *muons, *tracks, StandAlone_nt, match_muon_idx, match_trk_idx, *pv); 
...
// ------------
void TestMuonFullAODAnalyzer::beginJob() {
...
// ------------
void TestMuonFullAODAnalyzer::endJob() {}

// ------------ method fills 'descriptions' with the allowed parameters for the
// module  ------------
void TestMuonFullAODAnalyzer::fillDescriptions(edm::ConfigurationDescriptions& descriptions) {
...
DEFINE_FWK_MODULE(TestMuonFullAODAnalyzer);
