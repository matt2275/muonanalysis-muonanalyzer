...
bool StandAloneMuonFullAODAnalyzer::HLTaccept(const edm::Event& iEvent, NtupleContent& nt, std::vector<std::string>& HLTPaths) {
...
void StandAloneMuonFullAODAnalyzer::analyze(const edm::Event& iEvent, const edm::EventSetup& iSetup) {
  ...
  if (vertices->empty())
    return;
//  edm::Handle<std::vector<reco::Muon::AllStandaloneMuons>> SAmuons;
  ...
  for (auto& tag : tag_trkttrk) {
    ...
    // loop over probes
    for (const reco::Muon& probe : *muons) {
      auto probe_idx = &probe - &muons->at(0);

      // apply cuts on probe
       if (!probe.isStandAloneMuon())
        continue;
       if (!probeMuonSelection_(probe))
         continue;
      ...
      auto trk_it = std::find(trk_muon_map.second.begin(), trk_muon_map.second.end(), &probe - &muons->at(0));

      if (trk_it != trk_muon_map.second.end()){   
      unsigned idx = std::distance(trk_muon_map.second.begin(), trk_it);
      auto trk_probe = tracks->at(trk_muon_map.first[idx]);
      // compute vtx
      std::vector<reco::TransientTrack> trk_pair = {tag.second, reco::TransientTrack(trk_probe, &(*bField))};
      KlFitter vtx(trk_pair);
      
       if ((RequireVtxCreation_ && !vtx.status()) or (minSVtxProb_ > 0 && vtx.prob() < minSVtxProb_)){
      pair_vtx_probs.push(std::make_pair(-99, pair_idx));
      pair_dz_PV_SV.push(std::make_pair(-99, pair_idx));          
    ...
    for (const reco::Muon& probe : *muons) {
      if (debug_ > 1)
        std::cout << "    Probe pt " << probe.pt() << " eta " << probe.eta() << " phi " << probe.phi() << "  charge "
                  << probe.charge() << "probe vz" << probe.vz() << std::endl;
      ...
      if (trk_it == trk_muon_map.second.end()) {
        ...
        unsigned trk_idx = std::distance(trk_muon_map.second.begin(), trk_it);
        auto trk_probe = tracks->at(trk_muon_map.first[trk_idx]);
        unsigned idx = std::distance(trk_muon_map.second.begin(), trk_it);
        if (debug_ > 0)
      ...
      unsigned trk_idx = std::distance(trk_muon_map.second.begin(), trk_it);
      auto trk_probe = tracks->at(trk_muon_map.first[trk_idx]);
      auto itdsa = std::find(probe_dSA_map.first.begin(), probe_dSA_map.first.end(), &trk_probe - &tracks->at(0));
      auto itdsa_tag = std::find(tag_dSA_map.first.begin(), tag_dSA_map.first.end(), &tag - &tag_trkttrk[0]);
      auto itdgl = std::find(probe_dGl_map.first.begin(), probe_dGl_map.first.end(), &trk_probe - &tracks->at(0));
      auto itcosmic = std::find(probe_cosmic_map.first.begin(), probe_cosmic_map.first.end(), &trk_probe - &tracks->at(0));

      if (itdsa == probe_dSA_map.first.end()) {
        nt.probe_dsa_segmentMatches = -1;
        FillProbeBranchesdSA<reco::Track>(trk_probe, nt, false);
      } else {
      ...
      if (itdsa_tag == tag_dSA_map.first.end()) {
        ...
        FillTagBranchesdSA<reco::Track>(trk_probe, nt, false);
      } else {
      ...
      if (itdgl == probe_dGl_map.first.end()) {
        nt.probe_ndgl = 0;
        FillProbeBranchesdgl<reco::Track>(trk_probe, nt, false);
      } else {
      ...
      if (itcosmic == probe_cosmic_map.first.end()) {
        nt.probe_ncosmic = 0;
        FillProbeBranchesCosmic<reco::Track>(trk_probe, nt, false);
      } else {
      ...
      auto it_genmatch = std::find(matched_track_idx.begin(), matched_track_idx.end(), &trk_probe - &tracks->at(0));
      nt.probe_isMatchedGen = (it_genmatch != matched_track_idx.end());
      nt.probe_isHighPurity = trk_probe.quality(Track::highPurity);
      }
      nt.iprobe++;
       nt.pair_rank_vtx_prob = pair_rank_vtx_prob[{&tag - &tag_trkttrk[0], &probe - &muons->at(0)}];
       nt.pair_rank_dz_PV_SV = pair_rank_dz_PV_SV[{&tag - &tag_trkttrk[0], &probe - &muons->at(0)}];
       nt.pair_rank_dPhi_muons = pair_rank_dPhi_muons[{&tag - &tag_trkttrk[0], &probe - &muons->at(0)}];
       nt.pair_rank_dM_Z_Mmumu = pair_rank_dM_Z_Mmumu[{&tag - &tag_trkttrk[0], &probe - &muons->at(0)}];
